...
<div class="container">
  ...
  <div class="mb-4">
    <div class="d-flex justify-content-between align-items-center">
      ...
      <a href="{{ route('posts.index', ['bookmarked'=>1]) }}" class="btn btn-sm btn-outline-secondary">もっと見る</a>
    </div>
    <ul class="list-group mt-2">
      @forelse($bookmarkedPosts as $bp)
        <li class="list-group-item d-flex justify-content-between align-items-center">
          <a href="{{ route('posts.show', $bp) }}">{{ $bp->title }}</a>
          <small class="text-muted">{{ $bp->created_at->format('Y-m-d') }}</small>
        </li>
      @empty
